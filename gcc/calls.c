...
/* CYGNUS LOCAL sibcall/law */
  ...
  if (sibcall && HAVE_sibcall && HAVE_sibcall_value)
    {
      if (valreg)
...
/* END CYGNUS LOCAL */
  ...
  /* CYGNUS LOCAL sibcall/law */
  SIBLING_CALL_P (call_insn) = sibcall;
  /* END CYGNUS LOCAL */
     ...
     int ignore;
{
  ...
  /* CYGNUS LOCAL sibcall/law */
  /* We can not perform tail recursion optimizations if the call could
     possibly throw or if we're expanding a loop or another call.

     ?!? Future work.  We could peel the last iteration off the loop to
     provide another potential tail recursion site.  It would probably help
     GCC optimize itself.  */
  ...
  if (optimize
      ...
      && TREE_OPERAND (TREE_OPERAND (exp, 0), 0) == current_function_decl)
    {
	 ...
	 call expansion.  */
      ...
      /* Verify that each argument is safe for unsave_expr.  */
	...
	if (! safe_for_unsave (TREE_VALUE (p)))
      ...
      if (p == NULL_TREE)
	{
	  /* We have a potential tail recursive call site.  Expand the call
	     into a series of insns and store them into a CALL_PLACEHOLDER rtx.

	     Since we are going to expand the function call's arguments
	     multiple times we need to wrap them in an UNSAVE_EXPR to prevent
	     caching of info from multiple expansions.  */
	  for (p = actparms; p; p = TREE_CHAIN (p))
	    TREE_VALUE (p) = unsave_expr (TREE_VALUE (p));
	  ...
	  /* Emit the pending stack adjustments before we expand any
	     arguments.  */
	  ...
	  /* If the last insn on the tail recursion sequence is not a
	     BARRIER, then tail recursion optimization failed.  We must
	     reemit any block notes off the saved sequence, then throw
	     it away.  */
	  if (last == NULL_RTX || GET_CODE (last) != BARRIER)
	    {
	      reemit_block_notes_from_seq (get_last_insn (),
  ...
  for (pass = 0; pass < 2; pass++)
      ...
      if (pass == 0)
	{
	  ...
	  /* If P is nonzero, then some argument was not safe for unsave,
	     so we can not create a sibling call sequence.  */
	  if (p)
	    continue;

	  /* UNSAVE each argument expression.  */
	  for (p = actparms; p; p = TREE_CHAIN (p))
	    TREE_VALUE (p) = unsave_expr (TREE_VALUE (p));

	  /* State variables we need to save and restore between iterations.  */
	...
	do_pending_stack_adjust ();
	 ...
	 We do not support sibling calls when we pass the address of
	 the return value as a hidden argument.

	 Note that the code in the else-if clause modifies actparms, so we
	 must not execute this code for the sibcall pass.  */
      if (pass == 0 && (structure_value_addr && struct_value_rtx == 0))
	sibcall_failure = 1;
      else if (structure_value_addr && struct_value_rtx == 0)
	{
      ...
      else
	{
	 ...
	 incoming argument block.  */
      if (pass == 0)
	argblock = plus_constant (arg_pointer_rtx,
      ...
      if (pass != 0 && structure_value_addr && ! structure_value_addr_parm)
	{
	 ...
	 now!  */
      ...
      emit_call_1 (funexp, fndecl, funtype, args_size.constant,
		   struct_value_size,
		   FUNCTION_ARG (args_so_far, VOIDmode, void_type_node, 1),
	...
	current_function_calls_longjmp = 1, sibcall_failure = 1;
	 ...
	 if the user uses the `return' command in the debugger.  */
      ...
      if (is_volatile || is_longjmp || pass == 0)
	emit_barrier ();
