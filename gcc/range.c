     ...
     int loop_depth;
{
  ...
  for (insn = range_start; insn && insn != range_end; insn = NEXT_INSN (insn))
	...
	{
	  block = block_tmp;
	  COPY_REG_SET (range_live, BASIC_BLOCK (block)->global_live_at_start);
	}
     ...
     int range_max_regno;
{
  ...
  COPY_REG_SET (live, BASIC_BLOCK (0)->global_live_at_start);
  ...
  for (i = 0; i < old_n_basic_blocks; i++)
      ...
      if (stream)
	{
	  ...
	  bitmap_print (stream, BASIC_BLOCK (i)->global_live_at_start,
			" {", "}\n");
	}
    }
  ...
  for (insn = first_insn; insn; insn = NEXT_INSN (insn))
      ...
      if (insn == block_start)
	{
	  ...
	  COPY_REG_SET (BASIC_BLOCK (block)->global_live_at_start, live);
	}
     ...
     int range_max_regno;
{
  ...
  for (range = live_range_list; range; range = XEXP (range, 1))
	...
	{
	  regset bits = BASIC_BLOCK (block)->global_live_at_start;
	  ...
	  BASIC_BLOCK (block)->global_live_at_start = bits;
	}
     ...
     FILE *stream;
{
  ...
  for (i = 0; i < n_basic_blocks; i++)
    {
      ...
      range_block_orig[i].live_at_start = BASIC_BLOCK (i)->global_live_at_start;
  ...
  for (insn = first_insn; insn; insn = NEXT_INSN (insn))
      ...
      if (block_tmp >= 0 && basic_block != block_tmp)
	{
	  ...
	  COPY_REG_SET (live, BASIC_BLOCK (basic_block)->global_live_at_start);
	}
	  ...
	  if (NOTE_LINE_NUMBER (insn) == NOTE_INSN_LOOP_BEG)
	      ...
	      if (loop_start && loop_user_block == user_block)
		{
		  ...
		  if (!follow_insn)
		    ...
		    COPY_REG_SET (live, BASIC_BLOCK (block_tmp)->global_live_at_start);
