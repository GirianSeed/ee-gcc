     ...
     char *fname;
{
  ...
  if (opts->gen_obscured.enabled && opts->use_obscured.enabled)
    {
      if (CPP_OBSCURE_DIRECTORY_IS_DEFAULT (opts->gen_obscured.directory))
	opts->gen_obscured.directory = opts->use_obscured.directory;
      else if (CPP_OBSCURE_DIRECTORY_IS_DEFAULT (opts->use_obscured.directory))
	opts->use_obscured.directory = opts->gen_obscured.directory;
    }
     ...
     char **argv;
{
  ...
  if (argv[i][0] != '-') {
	...
	if (len == 12)
	  {
	    if (bcmp (p, "fgenobscured", 12) == 0)
	      {
		i += handle_genobscured_option (pfile, argc - i, argv + i) - 1;
		break;
	      }
	    else if (bcmp (p, "fuseobscured", 12) == 0)
	      {
		i += handle_useobscured_option (pfile, argc - i, argv + i) - 1;
		break;
	      }
	  }
      }
