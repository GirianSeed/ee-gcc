...
/* Linked list for extra dependancies in expression structure.  */

struct extra_list
{
  struct expr *expr;
  struct extra_list *next;
};

#define ALLOC_EXTRA_LIST   ((struct extra_list *)			\
			    gcse_alloc (sizeof (struct extra_list)))
...
struct expr
{
  ...
  /* Insn expr came from */
  rtx insn;
  /* List of other non-obvious dependancies.   ie, Libcall blocks often 
     use pseudo regs which are not exposed in the REG_EQUALs note.  */
  struct extra_list *extra_exprs;
...
static struct expr *hash_scan_set	     PROTO ((rtx, rtx, int, int));
...
static struct expr *insert_expr_in_table      PROTO ((rtx, enum machine_mode,
					      rtx, int, int));
     ...
     rtx x, insn;
{
  ...
  /* Is x unchanged up to the REG_LIBCALL note.  */
  if (note)
    insn = XEXP (note, 0);
...
/* Insert expression X in INSN in the hash table.
   ...
   Returns a pointer to the expression structure if one is found or
   allocated.  */
...
/* Scan pattern PAT of INSN and add an entry to the hash table.
   ...
   ALWAYS means we want to common something we wouldn't otherwise
   want to. This is usally an external dependance in a libcall block. 
   Returns the expr struct if its a gcse'ble innsn, otherwise NULL.  */

static struct expr *
hash_scan_set (pat, insn, set_p, always)
     rtx pat, insn;
     int set_p, always;
{
  rtx src = SET_SRC (pat);
  ...
  if (GET_CODE (dest) == REG)
    {
      ...
      rtx tmp;
      rtx extra_regs = NULL_RTX;
      struct extra_list *extra_exprs = NULL;
      int was_equal = 0;
      int was_retval = 0;
      rtx libcall_sequence = NULL_RTX;
      /* If we want to record expressions, and SRC is a register, look if
	 we have a REG_EQUAL note.  If so, and there is also a REG_RETVAL
	 note, we can use the REG_EQUAL note as the expression, and
	 re-issue the LIBCALL sequence later.  */
      if (! set_p
	  && GET_CODE (src) == REG)
	{
	  rtx equal_note = find_reg_note (insn, REG_EQUAL, NULL_RTX);
	  rtx retval_note = find_reg_note (insn, REG_RETVAL, NULL_RTX);
	  /* Check if this is a libcall sequence. If so save the LIBCALL code,
	     and use the REG_EQUAL as the expression.  If we are optimizing
	     for size, we ignore libcalls for now because code hoisting
	     doesn't understand what to do with them yet.  */
	  if (!optimize_size && equal_note && retval_note)
	    {
	      rtx tmp, i;
	      was_retval = 1;
	      src = XEXP (equal_note, 0);
	      libcall_sequence = XEXP (retval_note, 0);

	      /* Don't even try to common if we find a CODE_LABEL.  */
	      for (tmp = libcall_sequence; tmp != insn; tmp = NEXT_INSN (tmp))
		...
		extra_regs = libcall_other_reg (insn, equal_note);
	      /* Check if there are pseudos used that we don't handle yet.  */
	      for (i = extra_regs; i != NULL_RTX; i = XEXP (i, 1))
		{
		  rtx reg = XEXP (i, 0);
		  rtx tmp2;
		  int found = 0;
		  /* Find original definition of this register.  */
		  tmp = libcall_sequence;
		  if (find_last_value (reg, &tmp, libcall_sequence, 1))
		    {
		      if (GET_CODE (tmp) == INSN
		          && GET_CODE (PATTERN (tmp)) == SET
		          && REGNO (SET_DEST (PATTERN (tmp))) == REGNO (reg))
		        {
			  struct expr *exp;
			  /* Make sure it gets added to the list of 
			     expressions to common. If it isn't added,
			     we can't common this libcall block.  */
			  exp = hash_scan_set (PATTERN (tmp), tmp, 0, 1);
			  if (exp) 
			    {
			      struct extra_list *tmp, *last;
			      tmp = ALLOC_EXTRA_LIST;
			      tmp->expr = exp;
			      tmp->next = NULL;
			      /* Add tmp to end of the expr list */
			      if (extra_exprs == NULL)
			        extra_exprs = tmp;
			      else
			        {
				  for (last = extra_exprs; last->next != NULL;
				  			   last = last->next)
				    ;
				  last->next = tmp;
				}
		  ...
		  /* If we didn't find and add the defining expression, 
		     we can't common the LIBCALL.  */
		  if (!found)
		    {
		      /* Set back to original value and bail.  */
		      src = SET_SRC (pat);
		      break;
		    }
		}
	  ...
	  && (want_to_gcse_p (src) || always))
	{
	  ...
	  /* An expression is not available if its operands are
	     ...
	     If we're forcing this with the always flag, make it
	     not available too.  */
	  int avail_p = oprs_available_p (src, insn) && ! always;
	  struct expr *exp;

	  exp = insert_expr_in_table (src, GET_MODE (dest), insn, antic_p, 
	  			      avail_p);
	  /* Dont make a new copy if its already been copied.  */
	  if (exp && exp->insn == insn)
	    {
	      if (was_equal)
		exp->insn = copy_rtx (exp->insn);
	      if (was_retval)
	        {
		  exp->insn = libcall_sequence;
		  exp->extra_exprs = extra_exprs;
		}
	    }
	  return exp;
	}
     ...
     int in_libcall_block;
{
  ...
  if (GET_CODE (pat) == SET && ! in_libcall_block)
    hash_scan_set (pat, insn, set_p, 0);
     ...
     int pre;
{
  ...
  int regno = REGNO (reg);
  rtx pat, libcall, copied_expr, set;
  rtx first_new_insn;
  struct occr *p;

  start_sequence ();
  /* If there is a RETVAL on an occurence of this expression,
     we have to duplicate the entire LIBCALL sequence */
  if (libcall = find_reg_note (expr->insn, REG_LIBCALL, NULL_RTX))
    {
      rtx retval_note = XEXP (libcall, 0);
      rtx temp;
      rtx fn_address = 0;
      rtx fn_reg = 0;
      ...
      for (temp = expr->insn; temp != retval_note; temp = NEXT_INSN (temp))
        {
	  rtx body;
	  rtx n,i1;
	  rtx next;
	  struct extra_list *tmp;

	  if (GET_CODE (temp) == NOTE)
	    continue;

	  body = copy_rtx (PATTERN (temp));

	  if (GET_CODE (temp) == CALL_INSN)
	    {
	      i1 = emit_call_insn (body);
	    ...
	    i1 = emit_insn (body);
          set_block_num (i1, bb);
	  /* Replace any extra expressions in this insn */
	  for (tmp = expr->extra_exprs; tmp != NULL; tmp = tmp->next)
	    {
	      struct expr *exp = tmp->expr;
	      replace_rtx (i1, SET_DEST (PATTERN (exp->insn)), 
	      		   exp->reaching_reg);
	    }
	  if (first == 0)
	    first = i1;
	  if (temp == fn_address_insn)
	    fn_address_insn = i1;
	  if (REG_NOTES (temp))
	    REG_NOTES (i1) = copy_rtx (REG_NOTES (temp));
	}
      copied_expr = get_insns ();
      
      /* Duplicate the RETVAL part, only copy it into the new register.  */
      temp = SET_SRC (PATTERN (retval_note));
      pat = emit_move_insn (reg, temp);
      REG_NOTES (pat) = copy_rtx (REG_NOTES (retval_note));
      set_block_num (pat, bb);

      /* Set new insn values on the REG_RETVAL and REG_LIBCALL notes.  */
      temp = find_reg_note (pat, REG_RETVAL, NULL_RTX);
      XEXP (temp, 0) = first;
...
/* Checks to see that any extra dependant expressions are going to 
   be commoned too. These are the result of a libcall block. If any 
   fail, we can't common this one. 

   EXPR is the expression structure we wish to check for dependancies.
   
   Returns a 1 if all extra_exprs are commonable too, otherwise 0.  */

static int
forced_dependant_expressions (expr)
     struct expr * expr;
{
  struct extra_list *extra_list;
  if (expr == NULL || expr->extra_exprs == NULL)
    return 1;

  /* loop through all the extra expressions, check if they are
     redundant. Fail if anything isn't.  */

  for (extra_list = expr->extra_exprs; extra_list != NULL;
					    extra_list = extra_list->next)
    {
      struct expr *tmp = extra_list->expr;
      rtx insn = tmp->insn;

      /* If the dependant expression is commoned, and its going to be 
         issued BEFORE expr is issued, then its ok.  */
      if (TEST_BIT (temp_bitmap[BLOCK_NUM (insn)], tmp->bitmap_index) 
	  && (expr->bitmap_index > tmp->bitmap_index))
	{
	  if (!single_set (insn))
	    abort ();
	  /* Make sure this ones dependants are OK too.  */
	  if (! forced_dependant_expressions (tmp))
	    return 0;
	}
      else
        return 0;
      ...
      for (expr = expr_hash_table[i]; expr != NULL; expr = expr->next_same_hash)
	{
	  ...
	  for (occr = expr->antic_occr; occr != NULL; occr = occr->next)
	      ...
	      if (TEST_BIT (temp_bitmap[bb], indx))
		{
		  ...
		  if (forced_dependant_expressions (expr))
		    {
		      rtx retval_note = find_reg_note (insn, REG_RETVAL, 
		      				       NULL_RTX);
		      rtx equal_note = find_reg_note (insn, REG_EQUAL, 
		      				      NULL_RTX);
		      rtx org_insn = insn;
	              /* Leave a REG_RETVAL alone, and insert a NEW
		         version of insn for the copy, making the 
			 old one obsolete. The new one will not have the
			 REG_RETVAL, and since it overwrites the result,
			 the entire libcall block will be dead. 
			 We need to avoid changing the value in the RETVAL 
			 here because we haven't inserted the new copy of the
			 libcall block yet, and we will need it to copy in
			 gcse_pre_insert.  */
		      if (retval_note) 
		        {
			  /* Add insn to list of libcalls to be deleted.  */
			  delete_libcalls = gen_rtx_EXPR_LIST (VOIDmode, insn,
			        			       delete_libcalls);
			  insn = emit_insn_after (copy_rtx (PATTERN (insn)), 
						  insn);
			  set_block_num(insn, bb);
			  set = single_set (insn);
			}
			...
			{
			  occr->deleted_p = 1;
			  SET_BIT (pre_redundant_insns, INSN_CUID (org_insn));
			  changed = 1;
			  gcse_subst_count++;
			  if (gcse_file)
			    {
			  ...
			  if (retval_note)
			    {
			      /* Keep register set table up to date */
			      record_one_set (REGNO (SET_DEST (set)), insn);
			      REG_NOTES (insn) = 
		        ...
		        {
			  /* Dont delete libcall if its not a valid change.  */
			  delete_libcalls = XEXP (delete_libcalls, 1);
			  delete_insn (insn);
			}
		    }
		}
...
pre_gcse ()
{
  ...
  /* List of libcalls which pre_delete says should be deleted.  */
  delete_libcalls = NULL_RTX;
  ...
  /* If there are any libcalls to be deleted, remove them here.  */
  while (delete_libcalls != NULL_RTX)
    {
      rtx insn, ret, tmp;
      ret = XEXP (delete_libcalls, 0);
      insn = find_reg_note (ret, REG_RETVAL, NULL_RTX);
      insn = XEXP (insn, 0); 

      /* delete all the insns between the libcall and retval */
      while (insn != ret)
	{
	  tmp = NEXT_INSN (insn);
	  delete_insn (insn);
