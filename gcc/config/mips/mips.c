     ...
     enum machine_mode mode;
{
  ...
  switch (GET_CODE (addr))
	...
	if (! SMALL_INT (offset))
      ...
      /* fall through */
      /* CYGNUS LOCAL symbol_ref_5900/gavin */
...
/* A C compound statement to output to stdio stream STREAM the
   ...
   'D'  print second register of double register operand.
   'L'  print low-order register of double register operand.
        print low-order  bits of a const_double integer.
   'M'  print high-order register of double register operand.
        print high-order bits of a const_double integer.
     ...
     FILE *file;	/* file to write instructions instead of making RTL */
{
  ...
  if (TARGET_MIPS5900 && store_p)
    {
      int regno;
      rtx insn, sqnc_first_insn, sqnc_last_insn, set;
      tree attr;
      ...
      for (insn = get_insns (); insn != NULL_RTX; insn = NEXT_INSN (insn))
	{
	  if (GET_RTX_CLASS (GET_CODE (insn)) == 'i')
	    note_stores (PATTERN (insn), mips_check_reg_mode);
	}
      
      start_sequence();
      set_new_first_and_last_insn (sqnc_first_insn, sqnc_last_insn);
      if ((attr = lookup_attribute
	   ("register_precision", DECL_MACHINE_ATTRIBUTES (current_function_decl)))
	  != 0)
	for (; attr; attr = TREE_CHAIN (attr))
	  {
	    char *reg_expr = IDENTIFIER_POINTER (TREE_VALUE (TREE_VALUE (attr)));
...
mips_valid_type_attribute_p (type, attributes, identifier, args)
     ...
     tree args;
{
  /* The register_precision attribute is only useful on the r5900 for functions
     and methods.  */
  if (! (TARGET_MIPS5900
  ...
  /* register_precision attribute specifies what mode to save registers on 
     the stack with. */
  if (is_attribute_p ("register_precision", identifier))
    {
      int j;
      char *reg_expr = IDENTIFIER_POINTER (TREE_VALUE (args));
      tree reg_length = TREE_VALUE (TREE_CHAIN (args));
      enum machine_mode mode = VOIDmode;
      tree attr = DECL_MACHINE_ATTRIBUTES (type);
     ...
     int n_ready;
{
  ...
  if (!TARGET_MIPS5900 || reload_completed)
    return;
