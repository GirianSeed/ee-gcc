...
typedef struct cpp_obscured_header_option cpp_obscured_header_option;
typedef struct cpp_obscured_header_key cpp_obscured_header_key;
...
#define CPP_BUFFER_IS_MAIN(PFILE, BUFFER) ((BUFFER) == CPP_NULL_BUFFER(PFILE) - 1)

struct cpp_obscured_header_option
{
  int enabled;
  char *directory;
  struct cpp_obscured_header_key *keys;
};

struct cpp_obscured_header_key
{
  struct cpp_obscured_header_key *next;
  char *name;
};
...
/* Nonzero means pass all #define and #undef directives which we actually
  ...
  char *deps_target;
  struct cpp_obscured_header_option gen_obscured;
  struct cpp_obscured_header_option use_obscured;
};
