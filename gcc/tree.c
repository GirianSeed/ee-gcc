...
/* Return nonzero if it is safe to unsave EXPR, else return zero.
   ...
   It is safe to unsave EXPR if and only if EXPR has no embedded
   RTL_EXPRs.

   In effect, this function indicates whether or not it is possible
   to modify EXPR so that it can be expanded into RTL multiple times.  */
