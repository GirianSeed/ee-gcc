     ...
     cpp_reader *pfile;
{
  ...
  if (pbuf->is_obscured && ! pbuf->obscured_key_known)
     ...
     char *filename;
{
  ...
  cpp_obscure_indirect = 0;
  if (opts->use_obscured.enabled)
    {
      int f = use_obscured_header (pfile, filename);
      if (f != 0)
	return f;
    }
     ...
     struct include_hash *ihash;
{
  ...
  fp->rlimit = fp->buf + length;
  fp->is_obscured = decode_obscured_header (pfile);
  length = fp->rlimit - fp->buf;
  ...
  opts = CPP_OPTIONS (pfile);
  if (opts->gen_obscured.enabled
      && ! CPP_BUFFER_IS_MAIN(pfile, pfile->buffer) && ! fp->is_obscured)
