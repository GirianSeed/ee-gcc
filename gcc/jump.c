...
/* CYGNUS LOCAL sibcall/law */
static void
jump_optimize_1 (f, cross_jump, noop_moves, after_regscan, minimal)
     rtx f;
     int cross_jump;
     int noop_moves;
     int after_regscan;
...
/* END CYGNUS LOCAL */
  ...
  while (changed)
      ...
      for (insn = f; insn; insn = next)
	{
	  ...
	  /* CYGNUS LOCAL sibcall/law */
	  if (! minimal
	      /* We can't do this after reload has completed.  */
	      && ! reload_completed
	      && this_is_condjump && ! this_is_simplejump
