...
#define VU0_REGISTER_WINDOW_START	0xb0000C00
#define VU1_REGISTER_WINDOW_START	0xb1007000
...
struct vuregs buf;
struct defaults def = { {0.0, 0.0, 0.0, 1.0}, 0 }; /* VF0xyzw, VI0 values */
...
int main() {
    ...
    */
    ...
    memcpy((void*) vu0, &(vu[0]), VU_REG_END );

    /* Assert that attempts are overwriting VF00 and VI0 are futile */
    for(i=0; i<2; i++)
      {
	vu[i].f[0][0] = def.f[0];
	vu[i].f[0][1] = def.f[1];
