...
static const OPTION watchpoint_options[] =
{
  { {"watch-delete", required_argument, NULL, OPTION_WATCH_DELETE },
      '\0', "IDENT|all|pc|cycles|clock", DISPLAY_OPTIONAL("Delete a watchpoint"),
      watchpoint_option_handler, DISPLAY_OPTIONAL(NULL) },

  { {"watch-info", no_argument, NULL, OPTION_WATCH_INFO },
      '\0', NULL, DISPLAY_OPTIONAL("List scheduled watchpoints"),
      watchpoint_option_handler, DISPLAY_OPTIONAL(NULL) },
...
sim_watchpoint_install (SIM_DESC sd)
{
  ...
  /* generate more advansed commands */
  {
      ...
      for (interrupt_nr = 0; interrupt_nr <= watch->nr_interrupts; interrupt_nr++)
	{
      ...
      int_options[0].doc_name = DISPLAY_OPTIONAL("watch-cycles-ACTION");
      int_options[0].arg = "[+]COUNT";
      int_options[0].doc = DISPLAY_OPTIONAL(doc);
    }
    int_options[1].doc_name = DISPLAY_OPTIONAL("watch-pc-ACTION");
    int_options[1].arg = "[!]ADDRESS";
    int_options[1].doc =
      DISPLAY_OPTIONAL("Watch the PC, take ACTION when matches ADDRESS (in range ADDRESS,ADDRESS), `!' negates test");
    int_options[2].doc_name = DISPLAY_OPTIONAL("watch-clock-ACTION");
    int_options[2].arg = "[+]MILLISECONDS";
    int_options[2].doc =
      DISPLAY_OPTIONAL("Watch the clock, take ACTION after MILLISECONDS (`+' for every MILLISECONDS)");
