...
typedef enum sfmt {
 ...
 , FMT_CMPI, FMT_CMPEQ, FMT_CMPZ, FMT_DIV
 , FMT_DIVH, FMT_JC, FMT_JL, FMT_JMP
 , FMT_LD, FMT_LD_D, FMT_LDB, FMT_LDB_D
 , FMT_LDH, FMT_LDH_D, FMT_LD_PLUS, FMT_LD24
 , FMT_LDI8, FMT_LDI16, FMT_LOCK, FMT_MACHI_A
 , FMT_MULHI_A, FMT_MV, FMT_MVFACHI_A, FMT_MVFC
 , FMT_MVTACHI_A, FMT_MVTC, FMT_NOP, FMT_RAC_DSI
 , FMT_RTE, FMT_SETH, FMT_SLL3, FMT_SLLI
 , FMT_ST, FMT_ST_D, FMT_STB, FMT_STB_D
 , FMT_STH, FMT_STH_D, FMT_ST_PLUS, FMT_TRAP
 , FMT_UNLOCK, FMT_SATB, FMT_SAT, FMT_SADD
 , FMT_MACWU1, FMT_MSBLO, FMT_MULWU1, FMT_SC
} SFMT;
              ...
              CASE (0_144, 0) :
                {
                  ...
                  DECODE_SWITCH (0_144_0, val)
                    ...
                    CASE (0_144_0, 0) :
                      {
                        static const DECODE_DESC insns[16] = {
                          { I (INSN_DIV), E (FMT_DIV) }, { I (INSN_DIVH), E (FMT_DIVH) }, 
                          { I (INSN_X_INVALID), E (FMT_EMPTY) }, { I (INSN_X_INVALID), E (FMT_EMPTY) }, 
                          { I (INSN_X_INVALID), E (FMT_EMPTY) }, { I (INSN_X_INVALID), E (FMT_EMPTY) }, 
                          { I (INSN_X_INVALID), E (FMT_EMPTY) }, { I (INSN_X_INVALID), E (FMT_EMPTY) }, 
                          { I (INSN_X_INVALID), E (FMT_EMPTY) }, { I (INSN_X_INVALID), E (FMT_EMPTY) }, 
...
#define FLD(f) abuf->fields.cti.fields.fmt_bcl8.f
  EXTRACT_IFMT_BCL8_VARS /* f-op1 f-r1 f-disp8 */

  EXTRACT_IFMT_BCL8_CODE
...
#define FLD(f) abuf->fields.cti.fields.fmt_bcl24.f
  EXTRACT_IFMT_BCL24_VARS /* f-op1 f-r1 f-disp24 */

  EXTRACT_IFMT_BCL24_CODE
...
#undef FLD
  ...
  {
    CGEN_INSN_INT insn = entire_insn;
#define FLD(f) abuf->fields.fmt_cmpeq.f
  EXTRACT_IFMT_CMPEQ_VARS /* f-op1 f-r1 f-op2 f-r2 */

  EXTRACT_IFMT_CMPEQ_CODE

  /* Record the fields for the semantic handler.  */
  FLD (i_src1) = & CPU (h_gr)[f_r1];
  FLD (i_src2) = & CPU (h_gr)[f_r2];
  TRACE_EXTRACT (current_cpu, abuf, (current_cpu, pc, "fmt_cmpeq", "src1 0x%x", 'x', f_r1, "src2 0x%x", 'x', f_r2, (char *) 0));
...
#undef FLD
  ...
  {
    CGEN_INSN_INT insn = entire_insn;
#define FLD(f) abuf->fields.fmt_divh.f
  EXTRACT_IFMT_DIVH_VARS /* f-op1 f-r1 f-op2 f-r2 f-simm16 */

  EXTRACT_IFMT_DIVH_CODE

  /* Record the fields for the semantic handler.  */
  FLD (i_dr) = & CPU (h_gr)[f_r1];
  FLD (i_sr) = & CPU (h_gr)[f_r2];
  TRACE_EXTRACT (current_cpu, abuf, (current_cpu, pc, "fmt_divh", "dr 0x%x", 'x', f_r1, "sr 0x%x", 'x', f_r2, (char *) 0));
...
#define FLD(f) abuf->fields.fmt_sadd.f
  EXTRACT_IFMT_SADD_VARS /* f-op1 f-r1 f-op2 f-r2 */

  EXTRACT_IFMT_SADD_CODE
