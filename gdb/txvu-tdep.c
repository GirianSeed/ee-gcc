     ...
     struct ovly_linetab *linetab;
{
  ...
  for (n=GLOBAL_BLOCK; n<=STATIC_BLOCK; n++)
      ...
      if (p)
	{
	  ...
	  for (i=0; i<(b_nsyms - p->nsyms); i++)
	    {
	      if (BLOCK_SYM (block, i) == NULL)
		{
		  for (j=i+1; j<b_nsyms; j++)
  ...
  /* Now compress */
  for (n=STATIC_BLOCK+1; n<(BLOCKVECTOR_NBLOCKS (bv) - npending); n++)
    {
      if (BLOCKVECTOR_BLOCK (bv, n) == 0)
	{
	  for (i=n+1; i<BLOCKVECTOR_NBLOCKS (bv); i++)
