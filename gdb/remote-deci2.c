...
static int load_mem PARAMS((u_long addr, void *ptr, int len));
static int store_mem PARAMS((u_long addr, void *ptr, int len));
...
static int deci2_can_run PARAMS((void));
    ...
    struct timeval *timeout;
{
 ...
 retry:
	...
	if (LOAD(netmp_head->code) == NETMP_CODE_RESETR)
	    printf_filtered("Target hardware was reseted.\n");
    }
    ...
    char *arg;
{
    ...
    if (sscanf(port_num_str, "%d", &port) != 1 || !port)
	...
	error("deci2_init: can not connect to %s.", hp->h_name);
    ...
    struct target_waitstatus *status;
{
    ...
    DECI2HEAD *deci2 = &buffer.deci2head;
    DBGPHEAD *dbgp_head = &buffer.dbgp_head;
    int recv_len;
...
static int
deci2_map_regkind(regno)
    int regno;
{
    return regkind_map[regno];
}

static int
deci2_map_regno(regno)
    int regno;
{
    return regno_map[regno];
}
    ...
    int regno;
{
    ...
    CORE_ADDR *reg = (CORE_ADDR *)buffer.reg;
...
static unsigned long break_ins = 0x0001000d;
